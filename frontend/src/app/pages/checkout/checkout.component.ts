          ...
          <div class="grid lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            <div class="order-2 lg:order-1">
              ...
              <div class="flex items-center gap-4 mb-8">
                <span class="text-xs tracking-wider uppercase text-gray-500">Finalizar Pedido</span>
              ...
              <section class="mb-8">
                <div class="flex items-center justify-between mb-4">
                  <h2 class="text-xs tracking-wider uppercase font-medium">Contato</h2>
                  @if (!auth.isAuthenticated()) {
                    <a routerLink="/login" class="text-xs underline underline-offset-2">
              ...
              <!-- Shipping Section -->
              <section class="mb-8">
                <h2 class="text-xs tracking-wider uppercase font-medium mb-4">Endereço de Entrega</h2>
              ...
              <!-- Shipping Method -->
              <section class="mb-8">
                <h2 class="text-xs tracking-wider uppercase font-medium mb-4">Método de Envio</h2>
                ...
                <div class="space-y-3">
                  ...
                  >
                    ...
                    <span class="text-sm font-medium">
                      {{ shippingCost > 0 ? (shippingCost | currency:'BRL':'symbol':'1.2-2') : 'Grátis' }}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <span class="text-sm font-medium">
                      {{ 29.90 | currency:'BRL':'symbol':'1.2-2' }}
                    </span>
                  </div>
                </div>
              </section>
              ...
              <!-- Payment Method -->
              <section class="mb-8">
                <h2 class="text-xs tracking-wider uppercase font-medium mb-4">Pagamento</h2>
                  ...
                  >
                    ...
                    <div class="flex-1">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs bg-green-100 text-green-700 px-2 py-0.5">5% OFF</span>
                      </div>
                  ...
                  @if (paymentMethod === 'CREDIT_CARD') {
                    <div class="p-4 space-y-3">
                      ...
                      <input 
                        ...
                        class="w-full border border-gray-200 px-4 py-3 focus:border-black focus:ring-0"
                      />
                      <select class="w-full border border-gray-200 px-4 py-3 focus:border-black focus:ring-0">
                        <option>1x de {{ total | currency:'BRL':'symbol':'1.2-2' }}</option>
                        <option>2x de {{ total / 2 | currency:'BRL':'symbol':'1.2-2' }}</option>
                        <option>3x de {{ total / 3 | currency:'BRL':'symbol':'1.2-2' }}</option>
                        <option>4x de {{ total / 4 | currency:'BRL':'symbol':'1.2-2' }}</option>
                        <option>5x de {{ total / 5 | currency:'BRL':'symbol':'1.2-2' }}</option>
                        <option>6x de {{ total / 6 | currency:'BRL':'symbol':'1.2-2' }}</option>
                      </select>
                    </div>
                  }
              ...
              >
                @if (processing()) {
                  ...
                  Finalizar Pedido • {{ finalTotal | currency:'BRL':'symbol':'1.2-2' }}
                }
              </button>
            ...
            <div class="order-1 lg:order-2">
              <div class="lg:sticky lg:top-8">
                <h2 class="text-xs tracking-wider uppercase font-medium mb-6">Resumo do Pedido</h2>
                  ...
                  @for (item of cart.cartItems(); track item.id) {
                    <div class="flex gap-4">
                      <div class="relative w-20 h-24 bg-cream-50 flex-shrink-0">
                        <img 
                          [src]="item.imageUrl || 'https://placehold.co/80x96/f5f5f5/999?text=Sem+Imagem'" 
                      ...
                      <div class="flex-1 min-w-0">
                        <h3 class="text-sm font-medium truncate">{{ item.productName }}</h3>
                ...
                <div class="border-t border-gray-100 pt-4 space-y-2">
                  <div class="flex justify-between text-sm">
                    ...
                    <span>{{ cart.cartTotal() | currency:'BRL':'symbol':'1.2-2' }}</span>
                  </div>
                  <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Frete</span>
                    <span>{{ shippingCost > 0 ? (shippingCost | currency:'BRL':'symbol':'1.2-2') : 'Grátis' }}</span>
                  </div>
                  @if (paymentMethod === 'PIX') {
                    <div class="flex justify-between text-sm text-green-600">
                      <span>Desconto PIX (5%)</span>
                      <span>-{{ pixDiscount | currency:'BRL':'symbol':'1.2-2' }}</span>
                    </div>
                  }
                  <div class="flex justify-between text-lg font-medium pt-2 border-t border-gray-100">
                    <span>Total</span>
                    <span>{{ finalTotal | currency:'BRL':'symbol':'1.2-2' }}</span>
                  </div>
                </div>
              </div>
            </div>
...
export class CheckoutComponent {
  ...
  get shippingCost(): number {
    if (this.shippingMethod === 'express') return 29.90;
