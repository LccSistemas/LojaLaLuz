...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-cream-50 flex items-center justify-center py-12 px-4">
      <div class="max-w-md w-full">
        @switch (status()) {
          @case ('loading') {
            <div class="bg-white p-8 text-center">
              <div class="animate-spin w-12 h-12 border-2 border-black border-t-transparent rounded-full mx-auto mb-4"></div>
          ...
          @case ('invalid') {
            <div class="bg-white p-8 text-center">
              ...
              <h1 class="font-cormorant text-2xl mb-3">Link Inválido ou Expirado</h1>
          ...
          @case ('form') {
            <div class="bg-white p-8">
              ...
              <form (ngSubmit)="activateAccount()" class="space-y-6">
                <!-- Password -->
                <div>
                  <label for="password" class="block text-xs uppercase tracking-wider mb-2">
                    Senha
                  </label>
                  <input
                    type="password"
                ...
                <!-- Confirm Password -->
                <div>
                  <label for="confirmPassword" class="block text-xs uppercase tracking-wider mb-2">
                    Confirmar Senha
                  </label>
                  <input
                    type="password"
                ...
                <div class="text-xs text-gray-500 space-y-1">
                  ...
                  <p [class.text-green-600]="password === confirmPassword && password.length > 0">
                ...
                <button
                  ...
                  [class.opacity-50]="submitting() || password.length < 8 || password !== confirmPassword"
                >
          ...
          @case ('success') {
            <div class="bg-white p-8 text-center">
              ...
              <p class="text-gray-600 mb-8">
                ...
                para acompanhar seus pedidos e aproveitar benefícios exclusivos.
              </p>
              <div class="space-y-3">
                <a routerLink="/login" class="btn-primary block text-center">
                  Fazer Login
...
export class ActivateAccountComponent implements OnInit {
  ...
  verifyToken(): void {
    this.http.get<{valid: boolean}>(`${environment.apiUrl}/account/verify-token?token=${this.token}`)
      .subscribe({
        next: (response) => {
          if (response.valid) {
  ...
  activateAccount(): void {
    ...
    this.http.post<{message: string}>(`${environment.apiUrl}/account/activate`, {
      token: this.token,
      ...
      error: (err) => {
        this.error.set(err.error?.message || 'Erro ao ativar conta. Tente novamente.');
        this.submitting.set(false);
      }
    });
  }
